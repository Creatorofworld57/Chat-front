...
const AudioPlayer = ({ audioData }) => {
    ...
    const loadTrack = useCallback(async newCounter => {
        ...
        const newSound = new Howl({
            src: [`${backendUrl}/api/audio/${counter}`],
            volume: volume,
            format: ['mp3'],
        ...
        const response = await fetch(`${backendUrl}/api/audioName/${counter}`);
    ...
    const fetchImg = async () => {
        if (name) {
            try {
                const response = await fetch(`${backendUrl}/api/audio/image/${name}`);
                if (response.ok) {
    ...
    const forward = useCallback(async () => {
        if ((tracks === null || tracks.length === 0) && sound !==null) {
            sound.stop();
            setIsPlaying(false)
            const response = await fetch(`${backendUrl}/api/nextAudios?id=${counter}&direction=forward`);
            const newTracks = await response.json();
            setTracks(newTracks);
            if (newTracks.length > 0) {
